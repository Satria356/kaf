...
const fake = 'Farbot~Md'
const fake1 = 'Farbot~Md'
const fake2 = 'NiJadwal Sholat '
ownerNumber = ["18312576749@s.whatsapp.net"]
modelmenu = 'gif'
...
ucapanWaktu = 'Selamat siang'//'Pagi ️'
} else if (hour_now >= '10' && hour_now <= '14') {
ucapanWaktu = 'Selamat siang'//'Siang ️'
} else if (hour_now >= '14' && hour_now <= '17') {
ucapanWaktu = 'Selamat soree'//'Soree '
...
const wit = moment.tz('Asia/Jayapura').format('HH : mm : ss')
       ...
       kahfz.createMessage = async (jidnya, kontennya, optionnya) => {
		...
		const troli = {
                         ...
                         "orderTitle": 'Farbot~Md', 
                         "sellerJid": '18312576749@s.whatsapp.net'
                    } 
                          } 
                              } 
        ...
        "hydratedButtons": [
          {
            "urlButton": {
              ...
              "url": "https://github.com/Satria356"
            }
          },
          {
            "quickReplyButton": {
        ...
        "hydratedButtons": [
          {
            "urlButton": {
              ...
              "url": "https://wa.me/18312576749"
            }
          },
          {
                                       callButton: {
                                            displayText: 'Number Me',
                                            phoneNumber: '18312576749@s.whatsapp.net'
                                        }
                                    }, 
      ...
      "hydratedTemplate": {
        ...
        "hydratedButtons": [
          
          {
                                       callButton: {
                                            displayText: 'Number Me',
                                            phoneNumber: '18312576749s.whatsapp.net'
                                        }
                                    }, 
...
anuy = ` ${pushname}, Saya Farbot Multi Device
𝑻𝑰𝑴𝑬𝑹
 _*${ucapanWaktu}*_
𝑾𝑰𝑩  : _${wib}_
𝑾𝑰𝑻 : _${wit}_
...
kahfz.sendMessage(m.chat, { image: buff1,caption: '©Fardev' }, { quoted: m}).catch(e => {
...
case 'jadwaltv':
	      ...
	      case 'motivasi': case 'dilanquote': case 'bucinquote': case 'katasenja': case 'puisi': {
            ...
            break
case 'ana': case 'simi': {
if (args.length == 0) return m.reply(`Contoh: ${prefix + command} Text`)
let anu = await fetchJson(api('zenz', '/api/simisimi', { text : text }, 'apikey'))
...
case 'doaniat':
              ...
              teksnyaa = `NIAT SHOLAT ${query}\n${asw.result}[ *NIAT SHOLAT BY FARBOT - BOT* ]` 
...
ini_txt = args.join(" ")
 bufffer = await getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=apikey beli sendiri&text=${ini_txt}`)
                 kahfz.sendMessage(m.chat, { image: bufffer,caption: '©Fardev ' }, { quoted: m})
                 }
 break
	    case 'sc': {
                m.reply('Script : https://nekopoi.care')
            }
            break
                    case 'faq':
              const buttons = [
...
case 'cr': case 'k': {
             let vcard = 'BEGIN:VCARD\n' // metadata of the contact card
                    ...
                    + 'item1.TEL;type=CELL;type=VOICE;waid=18312576749:+18312576749\n' // WhatsApp ID + phone number
                    + 'item1.X-ABLabel:OwnerKontol\n'
                    + 'item2.EMAIL;type=INTERNET: farrr300@gmail.com\n'
                    ...
                    + 'END:VCARD'
                kahfz.sendMessage(m.chat, { contacts: { displayName: 'Fahri.', contacts: [{ vcardd }] } }, { quoted: m })
            }
            break
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
            ...
            case 'owner': case 'creator': {
                let vcard = 'BEGIN:VCARD\n' // metadata of the contact card
                    ...
                    + 'END:VCARD'
                kahfz.sendMessage(m.chat, { contacts: { displayName: 'Owner', contacts: [{ vcard }] } }, { quoted: troli })
            }
            break
			case 'kick': {
